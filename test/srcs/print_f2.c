...
/*   Updated: 2019/11/22 21:31:14 by stross           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_luf_help2(double x, int len, char *str)
{
	if (g_pf_flags.flag_zero && g_pf_flags.width > len)
		print_luf_help3(x, len, str);
	else
	{
		if (g_pf_flags.flag_space &&
				g_pf_flags.width <= len && !g_pf_flags.flag_plus && x > 0)
		{
			ft_putchar_g(' ');
			len++;
		}
...
void	print_luf(va_list arg)
{
	...
	if (g_pf_flags.flag_minus)
		print_luf_help1(x, len, str);
	else
		print_luf_help2(x, len, str);
	free(to_free);
}

void	print_fl(va_list arg)
{
	if (g_pf_flags.f && !g_pf_flags.lu)
		print_lf(arg);
	if (g_pf_flags.f && g_pf_flags.lu)
		print_luf(arg);
}
